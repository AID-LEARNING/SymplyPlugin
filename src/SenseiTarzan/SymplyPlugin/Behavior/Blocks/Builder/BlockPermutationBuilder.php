...
class BlockPermutationBuilder extends BlockBuilder
{
	...
	public function addTrait(ITrait $trait) : static
	{
		...
		foreach ($trait->toBlockProperty() as $property) {
	...
	public function getPropertiesTag() : CompoundTag
	{
		...
		return $properties
			...
			->setTag("properties", new ListTag(array_reverse(array_map(fn(BlockProperty $property) => $property->toNBT(), array_values($this->getProperties())))));
	}
